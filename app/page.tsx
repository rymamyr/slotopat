  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#030816] text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 md:px-10 md:py-14">
        <div className="mb-10 flex items-center justify-between rounded-full border border-white/10 bg-white/5 px-5 py-3 backdrop-blur-xl">
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <div className="mb-5 inline-flex rounded-full border border-cyan-400/20 bg-cyan-400/10 px-4 py-2 text-sm text-cyan-300">
              Canlı veri • Sanity destekli yapı
            </div>
            ...
            <p className="mt-6 max-w-2xl text-base leading-7 text-white/60 md:text-lg">
              Slotopat, sponsorları daha modern, daha düzenli ve daha güven veren
              bir yapıyla sunmak için tasarlandı.
            </p>
                ...
                <div className="rounded-[28px] border border-white/10 bg-[#0b1222] p-5">
                  ...
                  {featured.externalUrl && (
                    <a
                      ...
                      target="_blank"
                      className="mt-6 inline-flex rounded-2xl bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:scale-[1.02]"
                    >
                      Siteye Git
                    </a>
                  )}
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20 md:px-10">
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          {sponsors.map((s, i) => (
            <div
              key={s._id}
              className="group rounded-[28px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-cyan-400/40 hover:bg-white/[0.07]"
            >
              <div className="mb-5 flex items-center justify-between">
              ...
              {s.externalUrl && (
                <a
                  href={s.externalUrl}
                  target="_blank"
                  className="mt-6 inline-flex rounded-2xl border border-white/10 px-4 py-2 text-sm text-white/75 transition hover:border-cyan-400/40 hover:text-cyan-300"
                >
                  Detaya Git →
                </a>
              )}
